            ...
            return (
              <AnimateOnScroll key={item.id} delay={0.1 * index}>
                <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group mb-8 last:mb-0">
                  ...
                  {/* Timeline dot */}
                  <div className="flex items-center justify-center w-12 h-12 rounded-full border-4 border-[var(--color-bg)] bg-[var(--color-surface)] text-[var(--color-accent-500)] shadow-[var(--shadow-md)] shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10 transition-transform duration-[var(--duration-normal)] group-hover:scale-110 group-hover:bg-[var(--color-accent-500)] group-hover:text-[var(--color-bg)]">
                  ...
                  <div className="w-[calc(100%-4rem)] md:w-[calc(50%-3rem)] transition-transform duration-[var(--duration-normal)] md:group-odd:hover:translate-x-2 md:group-even:hover:-translate-x-2">
                    <Card hoverable className="p-6 md:p-8">
                      <div className="flex flex-col mb-4">
                        <span className="text-[var(--text-xs)] font-bold px-3 py-1 bg-[var(--color-surface-hover)] text-[var(--color-text-secondary)] rounded-[var(--radius-full)] w-fit mb-3">
                          {item.dateRange}
                        </span>
                        <h3 className="font-display font-bold text-[var(--text-xl)] text-[var(--color-text-primary)]">
                          {item.title}
                        </h3>
                        <div className="text-[var(--color-accent-500)] font-medium mt-1">
                          {item.organization}
                        </div>
                      </div>
